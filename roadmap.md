...
- **Alternatywa dla drag&drop na mobile (przenoszenie kolejności notatek) — świadomie pominięta**: odkryto, że `reorderNotes`/przeciąganie w obrębie siatki jest de facto martwe — `activeNotes` w `useNotes.ts` zawsze sortuje po `pinned`+`updatedAt`, więc przeciągnięcie notatki nie zmienia widocznej kolejności (komentarz w kodzie to potwierdza: „order isn't currently read by any sort”). Budowanie przycisków „przenieś w górę/dół” dla funkcji, która i tak nic nie zmienia, byłoby mylące. Drugi przypadek drag&drop — przenoszenie notatki do folderu — ma już przyciskową alternatywę (`FolderPicker` w menu „Więcej” na `NoteCard`), więc nie wymaga dodatkowej pracy. Jeśli w przyszłości ktoś zechce realnie działającą ręczną kolejność notatek, to osobne zadanie: trzeba dodać sortowanie po `order` jako opcję widoku.
