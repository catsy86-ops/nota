...
interface AddNoteBarProps {
  onAdd: (title: string, content: string, color: NoteColor, labels: string[], reminder: number | null, images: string[], checklist: ChecklistItem[]) => void;
  allLabels: string[];
...
export const AddNoteBar = forwardRef<{ expand: () => void }, AddNoteBarProps>(function AddNoteBar({ onAdd, allLabels, onCreateLabel }, ref) {
  ...
  function handleClose() {
    if (title.trim() || content.trim() || images.length > 0 || checklist.length > 0) {
      onAdd(title.trim(), content.trim(), color, labels, reminder, images, checklist);
    }
    reset();
  }

  function reset() {
    setTitle(""); setContent(""); setColor("default"); setLabels([]); setReminder(null); setReminderDate(undefined); setReminderTime("09:00"); setImages([]); setChecklist([]); setShowChecklist(false); setExpanded(false);
  }
